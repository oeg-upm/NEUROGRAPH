...
class LLMChat:
    ...
    @staticmethod
    def _primer_elemento_valido(array: list):
        """Devuelve el primer elemento que no sea None ni 'ERROR'."""
        for item in array:
    ...
    def _llamar_llm_para_campo(
        ...
        if not graph_data:
            data = (
                "No se encontraron datos. Seguramente sea un error por parte del usuario. "
                "Pregunta si se ha introducido bien el grupo."
            )
        else:
    ...
    def evaluate(
        ...
        for item in dataset:
            acierto, r_type, r_field = self._procesar_item_testeo(
    ...
    def _completar_siguiente_campo_llm(
        self,
        mis_datos: list,
        context_path: str,
        model: str,
        ...
        (versión del script v2testeo, sin interacción con el usuario).
        """
        if None not in mis_datos and "None" not in mis_datos:
            print(f"GraphRAG: query acabada → {mis_datos}")
            return mis_datos
        ...
        while retry:
            ...
            cleaned = self._llamar_llm_para_campo(
                mis_datos, cat_buscar, graph_data, context_path, model
            )
            output = self._primer_elemento_valido(cleaned)
    ...
    def _procesar_item_testeo(
        ...
        """Evalúa un único caso del dataset. Devuelve (acierto, ruletype, ruleField)."""
        ...
        result = self._completar_siguiente_campo_llm(mis_datospre, context_path, model)

        acierto = result == expected
        estado = "✅ ACIERTO" if acierto else "❌ FALLO"
        print(f"{estado} | RuleType: {ruletype} | RuleField: {ruleField}")
        if not acierto:
...
if __name__ == "__main__":
    ...
    resultados = chat.evaluate(
        ...
        dataset_path="./textos/datasetsantolimpio3.json", output_dir="./results", output_filename="res_eva4"
    )
