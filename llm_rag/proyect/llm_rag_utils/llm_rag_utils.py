...
class LLMChat:
    ...
    def __init__(
        ...
        # Carga del grafo RDF
        self.graph = Graph()
        self.graph.parse(graph_file_path, format=ttl_format)
    ...
    def _text_completion(self, prompt: str, engine: str = None) -> str:
        if engine is None:
    ...
    async def _text_completion_async(
        self, prompt: str, engine: str = None
    ) -> str:
        if engine is None:
            engine = self.model_name
        max_retry, retry = 5, 0
    ...
    async def _text_completion_batch(
        self, prompts_list: list[str], engine: str = None
    ) -> list[str]:
        if engine is None:
            engine = self.model_name
    ...
    def _llamar_llm_para_campo(
        ...
        data = (
                ...
                f"{self.DICCIONARIO_PREFIJOS[cat_buscar]}"
        ...
        reglas = formatear_para_llm(
            self.rules_path,
            tipo_cond=self.DICCIONARIO_PREDICADOS[cat_buscar],
        )
        prompt_base = self._open_file(context_path)
        mensajeinstruc = (
            "Se espera que extraigas el campo "
            + self.DICCIONARIO_PREFIJOS[cat_buscar]
        )
